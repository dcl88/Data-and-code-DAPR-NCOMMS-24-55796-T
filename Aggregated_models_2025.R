...
mydat5<- read.csv("Econ_wellbeing_biodiv_services_average_transf_2.csv", header=T)
head(mydat5)
names(mydat5) #gives the headings of every column

mydat6<- read.csv("Nonecon_wellbeing_biodiv_services_average_transf_2.csv", header=T)
head(mydat6)
names(mydat6) #gives the headings of every column

mydat7<- read.csv("Econ_wellbeing_suppreg_services_average_transf_2.csv", header=T)
head(mydat7)
names(mydat7) #gives the headings of every column

mydat8<- read.csv("Nonecon_wellbeing_suppreg_services_average_transf_2.csv", header=T)
head(mydat8)
...
#########################################################################################
###MODELS Biodiversity 
###4. Econ_wellbeing_average ~ Biodiv_services_average
###5. Nonecon_wellbeing_average ~ Biodiv_services_average 
# Fit GLM model 5 & 6
model_any_ECON5 <- glm(Econ_wellbeing_average_percent_transf ~ Biodiv_services_average_percent_transf, data = mydat5)
plot(Econ_wellbeing_average_percent_transf ~ Biodiv_services_average_percent_transf, data = mydat5)
summary(model_any_ECON5)

model_any_NONECON6 <- glm(Nonecon_wellbeing_average_percent_transf ~ Biodiv_services_average_percent_transf, data = mydat6)
plot(Nonecon_wellbeing_average_percent_transf ~ Biodiv_services_average_percent_transf, data = mydat6)
summary(model_any_NONECON6)

# Create sequences of values for prediction based on respective datasets
seq_ECON5 <- data.frame(Biodiv_services_average_percent_transf = seq(
  min(mydat5$Biodiv_services_average_percent_transf, na.rm = TRUE), 
  max(mydat5$Biodiv_services_average_percent_transf, na.rm = TRUE), 
  length.out = 100))

seq_NONECON6 <- data.frame(Biodiv_services_average_percent_transf = seq(
  min(mydat6$Biodiv_services_average_percent_transf, na.rm = TRUE), 
  max(mydat6$Biodiv_services_average_percent_transf, na.rm = TRUE), 
  length.out = 100))

# Predict values and confidence intervals for each model using the correct sequence
pred_ECON5 <- predict(model_any_ECON5, newdata = seq_ECON5, se.fit = TRUE)
pred_NONECON6 <- predict(model_any_NONECON6, newdata = seq_NONECON6, se.fit = TRUE)

# Create data frames for visualization
plot_data_any_ECON5 <- data.frame(
  Biodiv_services_average_percent_transf = seq_ECON5$Biodiv_services_average_percent_transf,
  pred_values = pred_ECON5$fit,
  X1 = pred_ECON5$fit - 1.96 * pred_ECON5$se.fit,  # Lower bound of CI
  X2 = pred_ECON5$fit + 1.96 * pred_ECON5$se.fit,  # Upper bound of CI
  WB = "Economic"
)

plot_data_any_NONECON6 <- data.frame(
  Biodiv_services_average_percent_transf = seq_NONECON6$Biodiv_services_average_percent_transf,
  pred_values = pred_NONECON6$fit,
  X1 = pred_NONECON6$fit - 1.96 * pred_NONECON6$se.fit,  # Lower bound of CI
  X2 = pred_NONECON6$fit + 1.96 * pred_NONECON6$se.fit,  # Upper bound of CI
  WB = "Non-economic"
)

# Combine both datasets
plot_data_all_WB_5_6 <- rbind(plot_data_any_ECON5, plot_data_any_NONECON6)
...
#Third set combined
# Plot the results with confidence intervals
p5 <- ggplot(plot_data_all_WB_5_6, aes(x = Biodiv_services_average_percent_transf, colour = WB, fill = WB)) +
  geom_line(aes(y = pred_values), col = "#CC79A7",linewidth = 1) +
  geom_point(data = mydat5, aes(x = Biodiv_services_average_percent_transf, y = Econ_wellbeing_average_percent_transf, colour = "Economic", fill = "Economic"), size = 1, shape = 21, stroke = 1) +
  geom_point(data = mydat6, aes(x = Biodiv_services_average_percent_transf, y = Nonecon_wellbeing_average_percent_transf, colour = "Non-economic", fill = "Non-economic"), size = 1, shape = 21, stroke = 1) +
  geom_ribbon(aes(ymin = X1, ymax = X2, fill = WB), alpha = 0.3) +
  labs(title = "(a) Wellbeing Outcomes from Biodiversity",
       x = "Biodiversity % Change",
       y = "Well-being (WB) % Change") +
  scale_fill_manual(values = c("Non-economic" = scales::alpha("#56B4E9", 0.4), "Economic" = scales::alpha("#E69F00", 0.4))) +
  scale_color_manual(values = c("Non-economic" = "#56B4E9", "Economic" = "#E69F00")) +
  theme_minimal() + ylim(-84, 600)
...
p5_a <- p5_a + theme(legend.position = "none", plot.title = element_blank()) + labs(y = NULL)
...
p5_a <- p5_a + theme(legend.position = "none", plot.title = element_blank()) + labs(y = "Well-being % Change")
